import type { Scale, ColorPalette, PlotArea } from "../charts/types.ts";
...
export function computeScale(min: number, max: number, tickCount = 5): Scale {
  ...
  const step = magnitude * (niceSteps.find((s) => s * magnitude >= roughStep) ?? 10);
...
export function drawAxes(
  ...
  // X axis
  canvas.hLine(area.left - 1, area.top + area.height, area.width + 1, "─", axisColor);
  // Corner
  ...
  for (const tick of yScale.ticks) {
    ...
    const yPos = area.top + area.height - 1 - Math.round(((tick - yScale.min) / yScale.range) * (area.height - 1));
  ...
  if (xLabels && xLabels.length > 0) {
    ...
    xScale.ticks.forEach((tick, i) => {
      ...
      const xPos = area.left + Math.round(((tick - xScale.min) / xScale.range) * (area.width - 1));
...
export function drawTitle(canvas: Canvas, title: string | undefined, palette: ColorPalette, noColor: boolean): void {
  if (!title) return;

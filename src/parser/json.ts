import type { DataPoint, Row, Series } from "../charts/types.ts";
import {
	parseNumeric,
	validateFieldExists,
	validateRows,
...
function parseJson(raw: string): unknown {
	...
	if (lines.length > 0) {
		return lines.map((line, i) => {
			try {
				...
				throw new Error(
					`Failed to parse line ${i + 1} as JSON: ${line.slice(0, 80)}`,
				);
			}
		});
	}
	...
	throw new Error(
		"Invalid JSON input. Expected a JSON array or NDJSON (one object per line).",
	);
}
...
export async function readInput(inlineJson?: string): Promise<Row[]> {
	...
	if (inlineJson) {
		...
		if (!raw) {
			throw new Error(
				"No input received on stdin. Pipe bq query output or use --json flag.",
			);
		}
	}

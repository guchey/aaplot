...
function renderBoxes(
  ...
  const area = computePlotArea(canvas.width, canvas.height - (hasMultiple ? 1 : 0), !!opts.title);
  ...
  const yLabels = boxes.map((b) => b.label);
  ...
  for (let bi = 0; bi < boxes.length; bi++) {
    const box = boxes[bi]!;
    const color = nc ? "" : (boxes.length > 1 ? getSeriesColor(palette, bi) : palette.bar);
    ...
    const toX = (v: number) =>
      area.left + Math.round(((v - xScale.min) / xScale.range) * (area.width - 1));
  ...
  if (hasMultiple) {
    ...
    drawLegend(canvas, boxes.map((b) => b.label), seriesColors, nc, legendRow);
  }

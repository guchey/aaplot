...
const SCHEMAS: Record<string, CommandSchema> = {
  bar: {
    ...
    required: { x: "string (field name, categorical)", y: "string (field name, numeric)" },
    optional: commonOptional,
    input: "JSON array via stdin or --json flag",
    input_example: [{ category: "A", count: "10" }, { category: "B", count: "25" }],
  },
  line: {
    command: "line",
    ...
    optional: commonOptional,
    input: "JSON array via stdin or --json flag",
    input_example: [{ date: "2024-01", value: "10" }, { date: "2024-02", value: "25" }],
  },
  area: {
    command: "area",
  ...
  scatter: {
    ...
    required: { x: "string (field name, numeric)", y: "string (field name, numeric)" },
    optional: commonOptional,
    input: "JSON array via stdin or --json flag",
    input_example: [{ distance: "1.5", fare: "10" }, { distance: "3.2", fare: "22" }],
  },
  histogram: {
    command: "histogram",
...
export function getSchema(command: string): string {
  ...
  if (!schema) {
    ...
    return JSON.stringify({ error: `Unknown command "${command}". Available: ${available}` }, null, 2);
  }

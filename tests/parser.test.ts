...
describe("extractXY", () => {
	...
	test("throws on missing x field", () => {
		const rows = [{ a: "1", b: "2" }];
		expect(() => extractXY(rows, "missing", "b")).toThrow(
	...
	test("throws on missing y field", () => {
		const rows = [{ a: "1", b: "2" }];
		expect(() => extractXY(rows, "a", "missing")).toThrow(
			'Field "missing" not found',
		);
	});
...
describe("extractColumn", () => {
	...
	test("throws on missing field", () => {
		const rows = [{ a: "1" }];
		expect(() => extractColumn(rows, "missing")).toThrow(
			'Field "missing" not found',
		);
	});
...
describe("groupRows", () => {
	...
	test("throws on missing group field", () => {
		...
		expect(() => groupRows(rows, "x", "val", "missing")).toThrow(
			'Field "missing" not found',
		);
	});

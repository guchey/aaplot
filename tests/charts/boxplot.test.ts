...
import { renderBoxplot, renderGroupedBoxplot } from "../../src/charts/boxplot.ts";
...
describe("renderBoxplot", () => {
  ...
  test("renders title when provided", () => {
    ...
    const result = renderBoxplot(values, "score", { ...defaultOpts, title: "Score Distribution" });
...
describe("renderGroupedBoxplot", () => {
  ...
  test("handles many groups", () => {
    ...
    const result = renderGroupedBoxplot(manyGroups, { ...defaultOpts, height: 25 });
    expect(typeof result).toBe("string");
  });
});

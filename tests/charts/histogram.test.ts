...
describe("renderHistogram", () => {
  ...
  test("uses custom title when provided", () => {
    const values = [1, 2, 3, 4, 5];
    const result = renderHistogram(values, "age", { ...defaultOpts, title: "Custom" });
  ...
  test("uses custom bin count", () => {
    ...
    const result10 = renderHistogram(values, "val", { ...defaultOpts, bins: 10 });
  ...
  test("handles large dataset", () => {
    const values = Array.from({ length: 1000 }, (_, i) => Math.sin(i) * 50 + 50);
    const result = renderHistogram(values, "val", defaultOpts);
    expect(typeof result).toBe("string");
  });

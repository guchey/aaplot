...
import { getAllSchemas, getSchema } from "../src/schema.ts";
...
describe("getAllSchemas", () => {
	test("returns valid JSON with all commands", () => {
		...
		expect(Object.keys(result.schemas)).toEqual([
			"bar",
			"line",
			"area",
			"scatter",
	...
	test("each schema has required fields", () => {
		const result = JSON.parse(getAllSchemas());
		for (const [key, schema] of Object.entries(result.schemas) as [
			string,
			// biome-ignore lint/suspicious/noExplicitAny: schema is dynamic JSON
			any,
		][]) {

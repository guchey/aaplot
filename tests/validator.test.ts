...
describe("rejectControlChars", () => {
	...
	test("accepts tabs, newlines, carriage returns", () => {
		expect(() =>
			rejectControlChars("line1\tvalue\nline2\r\n", "test"),
		).not.toThrow();
	});
...
describe("validateRows", () => {
	...
	test("throws on array of non-objects", () => {
		expect(() => validateRows(["string"])).toThrow(
			"Row 0: expected a JSON object",
		);
		expect(() => validateRows([null])).toThrow("Row 0: expected a JSON object");
		expect(() => validateRows([[1, 2]])).toThrow(
			"Row 0: expected a JSON object",
		);
	});
...
describe("parseNumeric", () => {
	...
	test("throws on Infinity", () => {
		expect(() => parseNumeric("Infinity", "val", 0)).toThrow(
			"is not a valid number",
		);
	});

	test("throws on NaN", () => {
		expect(() => parseNumeric("NaN", "val", 0)).toThrow(
			"is not a valid number",
		);
	});
});
